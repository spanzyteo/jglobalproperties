...
const SkeletonLandCard: React.FC = () => {
  return (
...
interface LandSkeletonProps {
  count?: number;
}

const LandSkeleton: React.FC<LandSkeletonProps> = ({ count = 1 }) => {
  if (count === 1) {
    return <SkeletonLandCard />;
  }

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 mt-4">
      {Array.from({ length: count }).map((_, index) => (
        <SkeletonLandCard key={index} />
      ))}
    </div>
  );
};
